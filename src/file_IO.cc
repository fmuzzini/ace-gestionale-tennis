...
const char DATA_PATH[] =	"data/";		/**< Directory dove il programma memorizza i dati */
...
static bool controlla_directory(const char path[])
{
	...
	file = g_strconcat(cartelle[0], "/", NULL);
	for (int i = 0; i < len; i++){
		...
		file_ = g_strconcat(file, cartelle[i+1], "/", NULL);
		g_free(file);
		file = file_;
	}
...
bool salva_circolo(const circolo_t *circolo)
{
	...
	dir = g_strconcat(DATA_PATH, nome, NULL);
	if ( !controlla_directory(dir) ) return false;

	//Apertura file per la scrittura
	file = g_strconcat(dir, DATI_CIRCOLO, NULL);
	ofstream f1(file);
...
circolo_t *carica_circolo(const char nome[])
{
	...
	char **campi = 0;
	int size = 0;
	char *file = g_strconcat(DATA_PATH, nome, DATI_CIRCOLO, NULL);
...
{
	char *dir = g_strconcat(DATA_PATH, circolo->nome->str, GIOCATORI_DIR, NULL);
	char *file = 0;
...
giocatore_t *carica_giocatore(const char file[], circolo_t *circolo)
{
	...
	//Creazione circolo
	giocatore = aggiungi_giocatore(campi[1], campi[2], campi[3], campi[4], campi[5], campi[6], campi[7], campi[8], circolo);
...
bool salva_campo(const campo_t *campo, const circolo_t *circolo)
{
	...
	char *dir = g_strconcat(DATA_PATH, circolo->nome->str, CAMPI_DIR, numero.str().c_str(), NULL);
	char *file = 0;
	...
	file = g_strconcat(dir, "/campo.txt", NULL);
	ofstream f1(file);
...
bool salva_ora(const ora_t *ora, const campo_t *campo, const circolo_t *circolo, bool archivia = false)
{
	...
	if (archivia)	
		dir = g_strconcat(DATA_PATH, circolo->nome->str, CAMPI_DIR, numero.str().c_str(), ARCHIVIO_DIR, NULL);
	else
		dir = g_strconcat(DATA_PATH, circolo->nome->str, CAMPI_DIR, numero.str().c_str(), ORE_DIR, NULL);
...
ora_t *carica_ora(char file[], campo_t *campo, circolo_t *circolo)
{
	...
	switch (tipo){
		case GIOCATORE:
			prenotante = cerca_lista_int(circolo->giocatori, ID, id, giocatore_t);
			break;
		case TORNEO:
			break;
		case CORSO:
...
bool backup(const char file[], circolo_t *circolo)
{
	...
	char *cartella = g_strconcat(DATA_PATH, circolo->nome->str, NULL);
